...
utf8dok supports corporate-style cover pages with configurable text overlays on background images. The cover system follows Asciidoctor PDF conventions for AsciiDoc compatibility.

=== AsciiDoc Attributes

Use standard document attributes to configure cover images:
...
:revnumber: 1.0.0
:revdate: 2025-12-31
:description: Complete guide to your project
:front-cover-image: image:covers/front.png[]
----

=== StyleContract Cover Section

For fine-grained control, define cover styling in `style-contract.toml`:

[source,toml]
----
[cover]
layout = "background"      # Image behind text
image_fit = "cover"        # Scale to fill page
image_position = "center"

[cover.title]
color = "FFFFFF"           # White text
font_size = 72             # 36pt (half-points)
bold = true
...
Cover elements support these template variables:
...
[source,bash]
----
# Render with cover image
utf8dok render document.adoc --cover cover.png --output out.docx
...
utf8dok supports generating PowerPoint presentations (PPTX) from AsciiDoc sources, using the same template-injection approach as DOCX. This enables "single source" workflows where documentation and presentations share content.

NOTE: PPTX generation is in development (Phase 1). Basic slide generation works; advanced features are coming.

=== Document Modes

==== Presentation Mode

For dedicated presentation documents:
...
Embed presentations within documentation using `[slides]` blocks:
...
* Multiple `[slides]` blocks → Concatenated presentation

=== Slide Syntax

utf8dok uses Asciidoctor Reveal.js compatible syntax:
...
|`= Title`
|Title Slide
|Presentation title (centered, large)
...
Add presenter notes with `[.notes]`:
...
* Emphasize sample size (n=1000)
* This slide: ~2 minutes
--
----

=== SlideContract

Like StyleContract for DOCX, SlideContract maps semantic types to PPTX layouts:
...
# Generate PPTX from dual-nature document
utf8dok render document.adoc --format pptx --output slides.pptx

# Generate both DOCX and PPTX
utf8dok render document.adoc --output doc.docx --slides slides.pptx
...
For detailed specification, see `architecture/specs/PPTX-GENERATION-SPEC.md`.
...
*Decision*: Configurable cover pages via StyleContract with Asciidoctor PDF attribute compatibility.

*Rationale*: Cover pages should be template-driven, not hardcoded. Aligns with existing `:front-cover-image:` attributes.

=== ADR-010: PPTX Dual-Nature Documents

*Decision*: Support PPTX generation with `[slides]` blocks for dual-nature documents.

*Rationale*: Single-source documentation with embedded presentations eliminates duplication. No existing AsciiDoc → PPTX tool exists.
...
* PPTX generation Phase 1 (basic slides)

=== Planned

* PPTX generation Phase 2-3 (dual-nature, images, diagrams)
* PDF output
